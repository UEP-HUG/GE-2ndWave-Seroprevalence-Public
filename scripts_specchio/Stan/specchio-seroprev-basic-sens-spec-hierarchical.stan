...
// There is no adjustment for household effects so ensure you pass the right data...
...
model {
    ...
    target+= normal_lpdf(sigma_logit_spec | 0, 1);//logit_spec_prior_scale); // weak truncated half-normal prior for spec variance
    target+= normal_lpdf(sigma_logit_sens | 0, 1);//logit_sens_prior_scale); // weak truncated half-normal prior for sens variance
